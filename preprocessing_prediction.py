...
def main():
    # parsing arguments:
    parser = argparse.ArgumentParser(description='ECG - makedataset')
    ...
    wfdir = args.path_signals
    freq_resample = args.freq_resampling
    dsdir = args.out_path
    random_seed = 42

    # Importing dataframe
    df = pd.read_csv(path_df, encoding="iso-8859-1", low_memory=False)
    ...
    labels = df["FA"].values
    filenames = np.array([os.path.join(wfdir, str(indx) + '.npy') for indx in exams])
    ...
    # Resample the signal to the required frqeuency
    ...
    # saving ECG matrix
    filename = os.path.join(dsdir, f'X_{n_samples}_12Leads_{freq_resample}hz')
    print(f'   Saving ECGs to file {filename}')
